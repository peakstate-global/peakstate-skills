...
| doc → parent | `brief-hash` `{hash}` | the document's hash changed; the host mirrors it into its own URL with `replaceState`. The host accepts only `#` plus `[A-Za-z0-9._~:-]`, 200 characters at most |
