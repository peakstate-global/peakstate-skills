...
**A box means "read this as one unit".** That is the whole rule, and it is why
there are exactly four of them: `details.example`, tables and `pre` (things read
as data), the answer textarea (a thing acted on), the **summary page**, the
**provenance block**, and a **figure with its caption**. The last three are
generated by the renderer, not authored — a figure gets its box because a chart
and the sentence explaining it are one thought, which is the reason the old
answer, indenting the pair, was wrong. Never add a fifth kind by hand.
